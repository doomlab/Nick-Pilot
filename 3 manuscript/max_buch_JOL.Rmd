...
tableprint = matrix(NA, nrow = 8, ncol = 4)

tableprint[ , 1] = c("Intercept", "Semantic Judgment", "Thematic Judgment", 
                     "Z Direct", "Z Indirect", "Z Interaction", "Z Direct Low", "Z Direct High")
tableprint[1:6 , 2:4] = summary(overallh2)$tTable[ , c(1,4,5)]

tableprint[7, 2:4] = summary(overallh2_low)$tTable[4, c(1,4,5)]
tableprint[8, 2:4] = summary(overallh2_high)$tTable[4, c(1,4,5)]

tableprint[ , c(2,3)] = printnum(as.numeric(tableprint[ , c(2,3)]))
tableprint[ , 4] = printp(as.numeric(tableprint[ , 4]))

colnames(tableprint) = c("Statistic", "Coefficient", "$t$", "$p$")

apa_table(as.data.frame(tableprint),
          align = c("l", rep("c", 3)), 
          caption = "MLM Statistics for Hypothesis 2",
          note = "Direct and indirect association  were mean centered. The table shows main effects and interactions predicting participant judgments. $df$ = 19404",
          escape = FALSE
)

```

```{r hyp2graph, echo=FALSE, fig.cap = "Simple slopes graph displaying the slope of direct association when predicting JORs at low, average, and high indirect association. All variables were mean centered.", fig.height=6, fig.width=6}
...
tableprint = matrix(NA, nrow = 9, ncol = 4)

tableprint[ , 1] = c("Intercept", "Semantic Judgment", "Thematic Judgment", "Judged Value",
                     "Z Direct", "Z Indirect", "Z Interaction", "Z Direct Low", "Z Direct High")
tableprint[1:7 , 2:4] = coef(summary(recalloverall))[ , -2]

tableprint[8, 2:4] = coef(summary(recalloverall_low))[5 , c(1,3,4)]
tableprint[9, 2:4] = coef(summary(recalloverall_high))[5 , c(1,3,4)]

tableprint[ , c(2,3)] = printnum(as.numeric(tableprint[ , c(2,3)]))
tableprint[ , 4] = printp(as.numeric(tableprint[ , 4]))

colnames(tableprint) = c("Statistic", "Coefficient", "$Z$", "$p$")


apa_table(as.data.frame(tableprint),
          align = c("l", rep("c", 4)), 
          caption = "MLM Statistics for Hypothesis 3",
          note = "Direct and indirect association  were mean centered. The table shows main effects and interactions predicting recall.",
          escape = FALSE
)

```

```{r hyp3graph, echo=FALSE, fig.cap = "Simple slopes graph displaying the slope of direct association when predicting recall at low, average, and high indirect association. All variables were mean centered.", fig.height=6, fig.width=6}
