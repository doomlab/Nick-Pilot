...
The JOL task can be manipulated to investigate perceptions of word pair relation by having participants judge how related they believe the cue and target items to be [@Maki2007a; @Maki2007]. The judged values generated from this task can then be directly compared to the normed databases to create a corresponding accuracy function or correlation as is created in JOL studies. When presented with the item pair, participants are asked to estimate the number of people out of 100 who would provide the target word when shown only the cue [@Maki2007], which mimics how association word norms are created through free association tasks. @Maki2007a investigated such judgments within the context of associative memory by having participants rate how much associative overlap was shared between normed item pairs and found that responses were greatly overestimated relative to the actual normed overlap strength for pairs that were weak associates, while underestimated for strong associates, thus replicating the @Koriat2005 findings for relatedness judgments based upon associative memory, rather than judgments based on learning.
...
# tiff(filename = "makislope-1.tiff", res = 150, width = 6, 
...
tableprint[ , c(2,3,4)] = printnum(as.numeric(tableprint[ , c(2,3,4)]), digits = 2)
...
tableprint2 = matrix(NA, nrow = 18, ncol = 7)

tableprint2[ , 1] = c(rep("Associative", 6), rep("Semantic", 6), rep("Thematic", 6))
tableprint2[ , 2] = c("Intercept", "Z Direct Association", "Z Indirect Association", "Z Interaction", "Z Direct Association Low", "Z Direct Association High",
                     "Intercept", "Z Direct Association", "Z Indirect Association", "Z Interaction", "Z Direct Association Low", "Z Direct Association High",
                     "Intercept", "Z Direct Association", "Z Indirect Association", "Z Interaction", "Z Direct Association Low", "Z Direct Association High")

colnames(tableprint2) = c("Judgment Condition", "Statistic", "Coefficient", "SE", "df","t", "p")

tableprint2[1:4, 3:7] = summary(overallh2.a)$tTable
tableprint2[5, 3:7] = summary(overallh2.alow)$tTable[2, ]
tableprint2[6, 3:7] = summary(overallh2.ahigh)$tTable[2, ]

tableprint2[7:10, 3:7] = summary(overallh2.s)$tTable
tableprint2[11, 3:7] = summary(overallh2.slow)$tTable[2, ]
tableprint2[12, 3:7] = summary(overallh2.shigh)$tTable[2, ]

tableprint2[13:16, 3:7] = summary(overallh2.t)$tTable
tableprint2[17, 3:7] = summary(overallh2.tlow)$tTable[2, ]
tableprint2[18, 3:7] = summary(overallh2.thigh)$tTable[2, ]

write.csv(tableprint2, "hyp2_by_condition.csv", row.names = F)
```
...
# tiff(filename = "hyp2graph-1.tiff", res = 150, width = 8, 
#     height = 6, units = 'in', compression = "lzw")
# hyp2graphout
# dev.off()
```
...
tableprint3 = matrix(NA, nrow = 21, ncol = 6)

tableprint3[ , 1] = c(rep("Associative", 7), rep("Semantic", 7), rep("Thematic", 7))
tableprint3[ , 2] = c("Intercept","Judged Value", "Z Direct Association", "Z Indirect Association", "Z Interaction", "Z Direct Association Low", "Z Direct Association High",
...
colnames(tableprint3) = c("Judgment Condition", "Statistic", "Coefficient", "SE", "t", "p")

tableprint3[1:5 , 3:6] = coef(summary(recalloverall.a))[ , ]
tableprint3[6, 3:6] = coef(summary(recalloverall_low.a))[3 , ]
tableprint3[7, 3:6] = coef(summary(recalloverall_high.a))[3, ]
tableprint3[8:12 , 3:6] = coef(summary(recalloverall.s))[ , ]
tableprint3[13, 3:6] = coef(summary(recalloverall_low.s))[3 , ]
tableprint3[14, 3:6] = coef(summary(recalloverall_high.s))[3, ]
tableprint3[15:19 , 3:6] = coef(summary(recalloverall.t))[ , ]
tableprint3[20, 3:6] = coef(summary(recalloverall_low.t))[3 , ]
tableprint3[21, 3:6] = coef(summary(recalloverall_high.t))[3, ]

write.csv(tableprint3, "hyp3_by_condition.csv", row.names = F)
```
...
tiff(filename = "hyp3graph-1.tiff", res = 150, width = 8, 
    height = 6, units = 'in', compression = "lzw")
hyp3graphout
dev.off()
```
...
tableprint[ , 2] = apa(as.numeric(tableprint[ , 2]),2,T) 
tableprint[ , 3] = apa(as.numeric(tableprint[ , 3]),2,T)
tableprint[ , 4] = apa(as.numeric(tableprint[ , 4]),2,T)
