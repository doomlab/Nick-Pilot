...
the strongest (\emph{b} = 0.896, \emph{p} = .090).
...
ability of associative word norms.

Our third hypothesis extended the analysis used in hypothesis 2 two
investigate the interaction between the three overlap norms when
predicting recall. As with the previous analysis, a significant
three-way interaction was found. Simple slopes analyses showed that this
interaction was in the same direction as when predicting recall, this
complementary effect was found when examining both recall and JORs.
...
(see Ferrand and New (2003) for a review), the presence of these
...
and meaning of words. The pathways between word properties are
bidirectional, allowing for feedback between connections. Harm and
Seidenberg (2004) developed a version which included a focus on
semantics, with word meaning being related to input from the orthography
and phonology components of the model. Our findings from the present
study further suggest that thematic and associative knowledge is
incorporated with meaning. One way of framing this is to consider the
semantic section of the triangle model as being comprised of these three
tiers, and that concept information is processed to some degree on each
of these domains. One area for future studies of this nature may be
investigating how aspects of orthography and phonology impact these
memory networks. Additionally, future studies may wish to consider
elements of thematic and associative knowledge when examining semantic
based tasks, such as word recognition and reading, as thematic and
associative information is interconnected with the semantic network.
Ultimately, further studies will be needed to explore the
interconnections between the semantic, thematic, and associative
networks.
